...
// GET FULL UNFOLLOW HISTORY (PRO ONLY) — GET /api/scans/unfollow-history
router.get('/unfollow-history', auth, async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    const plan = userCheck.rows[0]?.plan || 'free';

    if (plan === 'free') {
      return res.status(403).json({
        error: 'History tracking is a Pro feature',
        upgrade: true
      });
    }

    // Get all unfollowers across all scans, with dates
    const history = await pool.query(
      `SELECT uh.username, uh.detected_at, s.created_at as scan_date
       FROM unfollow_history uh
       JOIN scans s ON uh.scan_id = s.id
       WHERE uh.user_id = $1
    ...
    console.error('UNFOLLOW HISTORY ERROR:', err.message);
    res.status(500).json({ error: 'Server error' });
  }
});
